...
import com.ubirch.kafka.util.Implicits.enrichedInstant
...
trait WithNamespace {
  ...
  def prefixNamespace: String
  def metricsSubNamespaceLabel: String
  def metricsNamespace: String = if (simpleName) "" else prefixNamespace + "_" + metricsSubNamespaceLabel
}
